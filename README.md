...
Coverage that genuinely needs a real database or socket lives in a separate
integration workspace outside this repository. `docs/integration-test-backlog.md`
records every such property and what it must assert, so that work is deferred
rather than lost.
