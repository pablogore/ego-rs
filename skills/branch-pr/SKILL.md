---
name: ego-rs-branch-pr
description: "Create Ego pull requests with issue-first checks. Trigger: creating, opening, or preparing PRs for review."
license: Apache-2.0
metadata:
  author: pablogore
  version: "2.0"
...
- Open a pull request on [ego-rs](https://github.com/pablogore/ego-rs)
...
1. Confirm the issue has status:approved
   gh issue view <N> --repo pablogore/ego-rs
...
```bash
gh pr checks --repo pablogore/ego-rs <PR-number>
gh pr view --repo pablogore/ego-rs <PR-number>
```

### Add a Label

```bash
gh pr edit <PR-number> --repo pablogore/ego-rs --add-label "type:bug"
```
