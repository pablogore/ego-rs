...
No code in `security-sdk` or `service-sdk` may store `SecurityContext` in a thread-local, task-local (`tokio::task_local!`), or global (`static`, `once_cell`, `lazy_static`). The security field travels exclusively through explicit `ServiceContext` passing.

**Verification**: `grep -rn "thread_local\|task_local\|lazy_static\|once_cell::sync::Lazy" crates/security-sdk/src/ crates/service-sdk/src/context/` MUST return no security-context-related hits.
