# Spec: Atomic Read-Side Event Claiming

## Capability: read-side-event-claiming

Purpose: Defines the observable exclusion contract only: claim identity, acquisition refusal under a live claim, lease renewal, expiry-based takeover, stale-owner rejection via fencing, immediate release, ordering preservation, and the Production fail-closed gate. The exact port shape (`try_claim`/`renew`/`complete`/`release` or otherwise), storage mechanism (claim table vs. advisory lock), and migration number are design decisions (see Non-Goals).
...
- Prescribing the exact port method set, the storage mechanism (claim table,
  row lock, or advisory lock), or the migration number — design decisions.
