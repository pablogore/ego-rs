...
|------|------|-----------|----------------------|-----------------|-------------------|
| 1 | `Profile` + `require_configured` + `PersistenceCompositionError` in `persistent-entity` | PR 1 | `cargo test -p ego-persistent-entity profile::tests` | N/A — pure unit tests, no runtime needed | Delete `profile.rs`, the new error variant, and the `pub mod profile;` line; nothing depends on it yet |
| 2 | `EntityRuntimeBuilder::try_build()` + profile-gated `build()` | PR 2 | `cargo test -p ego-persistent-entity try_build` | N/A — unit-level, no real runtime | Revert `.profile()`, `validate_persistence()`, `try_build()`, and `build()`'s panic guard; `build()` reverts byte-for-byte, no call site touched |
...
- [ ] 5.1 Audit — using the ≥100-events-per-aggregate threshold (`PeriodicSnapshotStrategy::new(100)`), identify every Postgres integration test that can now trigger a real `PostgreSQLSnapshotStore::save_snapshot` (which calls `tokio::task::block_in_place`, panicking on a current-thread runtime). Start from the four named sites: `durable_entity_progress_postgres.rs:187,233,289,374`. Also check `single_aggregate_crash_recovery_postgres.rs`, `dual_aggregate_crash_recovery_postgres.rs`, `concurrent_replicas_postgres.rs`.
- [ ] 5.2 GREEN — migrate every test found in 5.1 that can cross the threshold from bare `#[tokio::test]` to `#[tokio::test(flavor = "multi_thread")]`; confirm the `tokio` `rt-multi-thread` feature is enabled for the crate under test (add to `Cargo.toml` if missing).
- [ ] 5.3 Verify (Docker-required) — run the migrated tests and confirm no `block_in_place` panic; confirm tests that stay under the threshold are unaffected by staying current-thread.
