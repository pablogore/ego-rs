...
|------|------|-----------|----------------------|-----------------|-------------------|
| 1 | `Profile` + `require_durably_configured` + `PersistenceCompositionError` en `persistent-entity`, más la declaración de capability `is_durable()` en `EventStore`/`Snapshot` | PR 1 | `cargo test -p persistent-entity profile::tests` | N/A — solo tests unitarios, sin runtime real | Borrar `profile.rs`, la nueva variante de error, los métodos de trait `is_durable()`, y la línea `pub mod profile;`; nada depende de esto todavía |
...
- [x] 1.1 RED — `crates/persistent-entity/src/profile.rs`: test tabulado `require_durably_configured_matrix` sobre {Dev, Production} × {durablemente configurado, no} (4 casos); falla al compilar.
- [x] 1.2 GREEN — crear `profile.rs`: enum `Profile` (`Dev` por defecto, `Production`), `require_durably_configured(profile, durably_configured, capability, fix)` (AD-1, AD-3). El parámetro está nombrado por lo que debe representar — durabilidad, nunca mera presencia — precisamente para que un call site que lo calcule solo con `.is_some()` se lea como incorrecto a simple vista.
...
- [x] 1.6 GREEN — agregar `fn is_durable(&self) -> bool { false }` como método default en `EventStore` (`crates/domain/src/persistence/event_store.rs`) y el método estructuralmente idéntico en `Snapshot` (`crates/domain/src/persistence/snapshot.rs`); sobreescribir a `true` en `PostgreSQLEventStore`/`PostgreSQLSnapshotStore`. Esta es la señal de durabilidad de la que cada caller de `require_durably_configured` calcula su booleano — el "pasar la respuesta, no el builder" de AD-3 solo se sostiene si la respuesta es honesta, y un método de trait propio del store es lo único que puede consultarse sin hacer downcast a un tipo concreto (lo cual además cegaría al gate ante cualquier implementación durable de terceros).
- [x] 1.7 RED — `crates/persistent-entity/src/profile.rs`: `presence_alone_is_not_durability` — fija la regresión exacta que un revisor señaló antes de que esto se enviara: `Some(InMemoryEventStore::new()).is_some()` y `Some(PostgreSQLEventStore::open(pool)).is_some()` no deben ser las dos entradas que recibe `require_durably_configured`.
...
- [x] 2.6 GREEN — agregar `.profile()`, `validate_persistence()` (event store verificado antes que snapshot store, según el orden de AD-3), `try_build()` (validar antes de delegar); `build()` llama a `validate_persistence()` y entra en pánico en `Err` (AD-4/AD-6). No se agrega ningún puente `From<PersistenceCompositionError>` — la negativa de event/snapshot vuelve solo al host (AD-6). Calcula sus dos booleanos `durably_configured` a partir de `self.event_store.as_ref().is_some_and(|s| s.is_durable())` / el equivalente de snapshot — nunca `.is_some()` (AD-3, 1.6).
...
- [x] 2.8 RED — `try_build_rejects_explicit_in_memory_event_store_under_production` y `try_build_rejects_explicit_in_memory_snapshot_store_under_production`: `Profile::Production` con un `InMemoryEventStore`/`InMemorySnapshotStore` cableado *explícitamente* debe rechazarse, no solo uno faltante — `is_some()` no puede distinguirlo de un store durable, solo `is_durable()` puede, y esto prueba que el gate realmente lo llama. Cada test aísla una capacidad con un `DurableStubEventStore`/`DurableStubSnapshotStore` (declara `is_durable() -> true`, cualquier otro método `unreachable!()`) en el lugar de la *otra* capacidad, para que un `InMemory*` ahí no enmascare en silencio cuál chequeo es el que el test está probando.
- [x] 2.9 GREEN — la implementación de 2.6 ya satisface 2.8; no hace falta ningún cambio de código adicional.
- [x] 2.10 Desviación encontrada y corregida — `try_build_rejects_missing_snapshot_store_under_production` y la mitad "solo evento" de `try_build_rejects_partial_configuration_under_production` (2.2/2.3) cableaban cada una un `InMemoryEventStore` explícito de manera incidental, para aislar el check del snapshot store. Bajo la regla de durabilidad, ese event store en memoria queda rechazado por sí mismo (se chequea primero, según AD-3), lo que habría cambiado el mensaje de error esperado en ambos tests de "snapshot store" a "event store". Se corrigió cableando el `DurableStubEventStore` de 2.8 en su setup, preservando intacta la intención de aislamiento original de cada test; ninguna aserción fue modificada.
...
- [x] 3.4 GREEN — agregar el campo `profile` + `.profile()` a `RuntimeBuilder`; agregar `validate_persistence_profile()` (AD-5), llamado desde `build()` y `try_build()` en el mismo lugar que `validate_idempotency()`; verifica solo `effect_state_store` (no `effect_dedup_store` — ambos siempre se configuran juntos mediante `with_effect_store`, según el `debug_assert_eq!` existente). Calcula su booleano `durably_configured` a partir de `self.effect_state_store.as_ref().is_some_and(|s| s.capabilities().durable)` — reutiliza el `EffectStoreCapabilities` ya existente de PROD-002 (ya `durable: false` por defecto, ya `true` en cada store respaldado por Postgres) en vez de agregar un método de trait nuevo; nunca `.is_some()` (AD-3).
...
- [x] 3.8 RED+GREEN — `try_build_rejects_an_explicit_in_memory_effect_store_under_production`: `Profile::Production` con un executor registrado y un `InMemoryEffectStore` cableado *explícitamente* debe rechazarse, no solo uno faltante — la contraparte de 2.8 para el effect store, probando que `validate_persistence_profile()` lee `capabilities().durable` y no la presencia.
...
**Nota de reconciliación**: esta fase implementó originalmente el fix de configuración-vs-durabilidad
(`is_durable()`, el renombre de `require_configured` a `require_durably_configured`, los dos stubs de
test `DurableStub*`, y el paso de deduplicación de la migración) como un agregado de WU8, encontrado
por `/code-review` después de que `sdd-verify` ya había devuelto PASS sobre WU1–WU7. La instrucción
explícita del arquitecto fue no enviar el fix como un parche posterior: `require_durably_configured` y
`is_durable()` debían ser el diseño que las Fases 1–3 enviaran desde el principio, no algo agregado seis
PRs después. El fix se reubicó ahí (ahora 1.6, 1.7, 2.6, 2.8–2.10, 3.4, 3.8, y el paso de deduplicación
de la migración plegado en 5.3) vía `git merge` propagado hacia adelante por cada unidad de trabajo
posterior, de modo que ningún PR de la pila final contenga jamás la regla más débil, solo-presencia —
ni siquiera transitoriamente. Lo que queda aquí es solo re-verificación.

- [x] 9.1 Confirmar que todos los tests de los que depende el fix reubicado pasan con el mecanismo
  ahora viviendo en su origen: `cargo test -p persistent-entity` y `cargo test -p ego-service-sdk`.
- [x] 9.2 Verificación (requiere Docker) — volver a correr `run-suite`; confirmar que la migración sigue
  aplicándose limpiamente y que todos los conteos se mantienen (43 pasados / 0 fallados / 1 ignorado).
- [x] 9.3 Gates finales — `cargo fmt --check`, `cargo check --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `run-suite` de Docker.
