...
- [ ] `AllowAllAuthorizationProvider` and `DenyAllAuthorizationProvider` implement `AuthorizationProvider`, are public, `Send + Sync`, object-safe, `Arc<dyn>`-injectable.
- [ ] AllowAll → `Ok(Allow)`; DenyAll → `Ok(Deny { reason })`, covered by tests written before impl.
