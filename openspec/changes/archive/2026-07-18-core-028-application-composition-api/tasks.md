...
- [x] 5b.1 (Integration gap) `ego_transport::AppState::new` requires runtime access for its own generic per-request `resolve::<Tag>()` dispatch, but `App`/`RunningApp` never exposed the inner `Runtime` — this transport layer predates `App`/`AppBuilder` and was never covered by PR1's design. Original fix added `#[derive(Clone)]` to `Runtime` and `App::runtime(&self) -> Runtime`; **superseded by 5b.2** after review found it leaked lifecycle authority.
- [x] 5b.2 (PR2 review, HIGH — corrected) `App::runtime() -> Runtime` let any holder of the returned handle call `Runtime::start_effects`/`shutdown_async`/`register_async_teardown` directly — the exact lifecycle surface `App`'s `start()`→`RunningApp`→`shutdown()` typestate exists to gate, confirmed reachable by grepping `impl Runtime`'s method visibility. Replaced with a narrower `RuntimeResolver` type (`crates/service-sdk/src/runtime/builder.rs`) exposing only `resolve::<Tag>()`/`logger()` — the two capabilities `ego_transport`'s handlers actually call (confirmed via `handlers.rs`: `state.runtime.resolve::<RegisterUserTag>()`, `state.runtime.logger()`). `Runtime::resolver()` constructs one from any `Runtime` (including ones built directly via `RuntimeBuilder`, outside `App` — `crates/transport/tests/security_extractor.rs` and `crates/service-sdk`'s own `effects_e2e.rs`-style direct consumers keep full `Runtime` access, since `RuntimeBuilder`/`Runtime` remains the infra API per AD-1/G2). `App::runtime()` renamed to `App::resolver()`; `ego_transport::AppState`'s `runtime` field changed from `Arc<Runtime>` to `RuntimeResolver`. Regression test: `app_resolver_sees_the_same_registered_logger_as_the_underlying_runtime` (`crates/service-sdk/src/app/mod.rs`).
- [x] 5b.3 (PR2 review, MEDIUM) `App::start()` didn't roll back on `start_effects` failure — a caller who registered a shutdown participant (e.g. a spawned read-side scheduler) via `App::register_shutdown` before calling `start()` would leak it if `start()` then failed, since no `RunningApp` exists to call `.shutdown()` on. Fixed: `App::start()` now runs `shutdown_async` on the failed attempt before returning the original `CompositionError::Startup`, draining every already-registered participant. Currently unreachable in the reference app (no effect executors registered, so `start_effects` always succeeds) but is now correct for the general, documented contract.
