...
- [x] 6.1 RED — `crates/testkit/src/effects.rs`: scripted per-op transient-error queue (`fail_calls: HashMap<StoreOp, VecDeque<EffectStoreError>>`) drives the retry path identically to a real transient failure. Evidence: `scripted_transient_mark_in_flight_failures_are_absorbed_by_the_real_delivery_runner_retry_path` drives a real `RuntimeEffectAcceptor` (Inline mode) with two scripted `MarkInFlight` `TemporarilyUnavailable` failures; `mark_in_flight_with_retry`'s real bounded retry absorbs both, executor dispatched exactly once. `scripted_failures_are_consumed_in_fifo_order_then_fall_through_to_the_real_store` proves FIFO consumption and real fall-through directly at the store level. Both pass (`cargo test -p ego-testkit effects::`).
- [x] 6.2 RED — `simulate_process_crash()` destroys all volatile state; a subsequent `recover_in_flight`/`claim_due` sees **nothing** recoverable — models `InMemoryEffectStore`'s documented non-durable loss. Not the operation recovery-logic tests use. Evidence: `simulate_process_crash_destroys_all_volatile_state` — asserts `claim_due` empty, `recover_in_flight` returns 0, `mark_succeeded` → `NotFound`, and a repeated `reserve` on the same scope sees `Fresh`, not a survivor. Passes.
- [x] 6.3 RED — `simulate_runner_crash()` preserves all backing state but abandons in-flight ops (as if the runner holding them died); a subsequent `recover_in_flight`/`claim_due` **does** see pre-crash in-flight effects as recoverable/reclaimable. This is the operation recovery-logic tests use, not `simulate_process_crash()`. Evidence: `simulate_runner_crash_preserves_backing_state_so_recovery_can_reclaim_it` — accept → `mark_in_flight` → `simulate_runner_crash()` → `recover_in_flight` returns 1 → `claim_due` sees it → redispatch `mark_in_flight` succeeds. Passes.
- [x] 6.4 RED — claim-race interleave hook: two `claim_due` → `mark_in_flight` sequences never both hold a valid claim; once a lease "expires" (scripted), redispatch is possible. Evidence: `claim_due_and_mark_in_flight_together_guarantee_exclusivity_and_post_abandonment_redispatch` — no interleave hook was needed (per design.md AD-6/AD-12, no owner/epoch/lease concept crosses the port); claimant A's `mark_in_flight` succeeds, claimant B's subsequent `claim_due` sees nothing and a direct `mark_in_flight` retry fails with `InvalidTransition`, then `simulate_runner_crash()` + real `recover_in_flight()` + `claim_due()` prove redispatch after abandonment. Passes — fully representable via the real InMemoryEffectStore mechanism, no fabricated lease machinery.
- [x] 6.5 RED — `crash_after(op)`: the write for `op` lands but its `Ok` never reaches the caller (ambiguity-window / idempotency-on-retry test), backed by the `FaultPlan::crash_after` field. Evidence: `crash_after_commit_success_hides_a_landed_write_but_the_real_retry_reconciles_it` — arms `crash_after(StoreOp::CommitSuccess)`, drives a real `RuntimeEffectAcceptor` (Inline mode); `finish_success`'s own bounded retry loop reconciles the hidden success (executor called exactly once, `accept()` returns `Ok`, a probe `reserve()` on the same scope sees `OtherSucceeded`, proving the first hidden write really landed and no corruption/double-execution occurred). Passes.
- [x] 6.6 GREEN — implement `FaultInjectingEffectStore` wrapping a real `InMemoryEffectStore`, both ports, `FaultPlan`/`StoreOp` (design §3.5), and all three crash operations — `simulate_process_crash()`, `simulate_runner_crash()`, `crash_after(op)` — no randomness (determinism axiom). Implemented in `crates/testkit/src/effects.rs`; `StoreOp` trimmed to the 8 ops `DeliveryRunner` actually calls (`ClaimDue`, `MarkInFlight`, `MarkSucceeded`, `MarkRetryable`, `MarkTerminal`, `Reserve`, `CommitSuccess`, `Release` — `Accept`/`RecoverInFlight` excluded, no `DeliveryRunner` call site exists for either). Exported from `ego-testkit`'s crate root.
- [x] 6.7 Add `FaultInjectingEffectStore` as an additional Phase 3 Tier 1 harness subject — proves it stays a genuine `EffectStateStore + EffectDedupStore`. Added `ego-testkit` as a dev-dependency of `ego-effect-store` (one direction only, no cycle) and a `tier1_fault_injecting` module in `crates/effect-store/tests/conformance.rs` running `run_state_store_conformance`/`run_dedup_conformance` against it with an empty fault plan. Passes with and without `--features stoolap`.
