...
- [x] 8.1 `cargo test --workspace` zero new failures; `cargo clippy --workspace -- -D warnings` clean; confirm no touched function exceeds cognitive-complexity 10.
- [x] 8.2 Re-run the full conformance suite (`cargo test -p ego-integration-tests --test read_side_progress_postgres`); confirm 3.1–3.8 all GREEN, including 3.7 now that Phase 6 exists.
- [x] 8.3 Diff-read confirmation (no code change): SC-7 — every `$N` bound, no interpolation anywhere; SC-11 — `crates/domain/src/read_side/`, `crates/service-sdk`'s gate/registration, and `crates/runtime/src/read_side/scheduler.rs` appear in no file list of this change.
