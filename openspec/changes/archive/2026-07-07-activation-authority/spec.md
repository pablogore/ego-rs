# Spec: CORE-006A — Activation Authority & Linearizability
...
reactivation-from-`Passivated`. New capability: `entity-activation`. No
existing spec under `openspec/specs/` is modified.
...
MUST leave an enqueued command permanently unanswered. This extends
FR-006/FR-007, which today cover only activation-time failure (recovery
error, spawn failure) and are silent on post-activation termination — the
gap two independent Judgment Day review rounds proved can strand
already-enqueued callers forever.
...
- Runtime bootstrap, dependency injection, transports, schedulers/scheduling
  policy, clustering (CORE-007), persistence protocol, authorization,
  tenancy, telemetry.
- Renaming existing concepts (e.g. "registry") — misleading terminology is
  documented, not renamed, by this change.
...
| Finding | Covered By |
|---|---|
| 1 — Registry visible before Active | FR-002, FR-003, FR-004 |
| 2 — Two independent sources of truth | FR-002 |
| 3 — `SharedActivation`/`Supervisor` dead code | Non-Goal (primitive choice); disposition deferred to design (D2) |
| 4 — Concurrent activations not serialized | FR-001, FR-005 |
| 5 — Activation not linearizable | FR-005 |
| 6 — Partial activation observable | FR-003, FR-004, FR-006 |
| 7 — Rollback not centralized | FR-006, FR-007 |

### Judgment Day Findings (Rounds 1–2, against design.md)

| Finding | Covered By |
|---|---|
| JD-1 — FR-006/FR-007 scoped only to activation-time failure; post-activation termination (panic/cancellation/shutdown) can strand already-enqueued callers forever, unresolved `oneshot`s | FR-009 |
| JD-2 — `MailboxClosed` during a passivation-to-reactivation handoff is indistinguishable from a permanent failure | FR-010 |
| JD-3 — Type-mismatch/downcast failure on an existing live entry could be treated as "no live entry," spawning a competing actor | FR-001 (strengthened) |

## Assumptions

- D1: `active_count()`'s visible semantics MAY change (excludes `Recovering`)
  — today's early-visible behavior is a bug, not a contract to preserve.
- D2: Disposition of `activation.rs`/`supervisor.rs` (reuse, rewrite, or
  removal) is a design-phase decision; this spec requires only that a single
  activation authority exists.
- D3: FR-003, FR-004, and FR-005 apply uniformly to cold activation and
  reactivation-from-`Passivated`; no separate contract exists by origin.
- D4: `ARCHITECTURE.md` alignment lands in the same change; this is a
  delivery decision tracked at tasks/apply, not a runtime observable
  contract itself.
