...
- [x] TASK-020 RED+GREEN: panic mid-processing while `Active` with N queued commands — assert all N `rx.await` resolve to a terminal `Err`. **Already fully covered** by Phase 3's `actor.rs` unit test `panic_mid_processing_answers_all_already_enqueued_callers` (TASK-008) — same actor state (`Active`), same "N queued behind a panicking one" shape, same assertion; writing it again would be a pure duplicate. This slot's checkbox instead covers the genuinely missing adjacent scenario the apply pass's framing called out: `panic_during_recovery_answers_enqueued_caller_and_leaves_no_zombie` — a panic during `recover_state()` itself (before the actor ever reaches `Active`), proven through the real production entry point (`EntityRuntime::entity_ref` → `TokioEntityRef::new`), not a hand-built actor.
- [x] TASK-021 RED+GREEN: panic/cancellation mid-passivation-drain — assert the undrained remainder still gets a terminal reply, independent of the in-body drain loop. Implemented as `panic_mid_processing_after_real_successes_answers_the_remainder`, deliberately exercised through `process_commands()`'s own loop (current_thread flavor, fully deterministic) rather than racing the real passivation timer into `passivate()`'s specific drain loop — ADR-005 states the guard's drain is "independent of the normal drain loop" and drains whatever is left in the mailbox's `VecDeque` regardless of which in-body loop was iterating, so the two loops are interchangeable for what this proves. See the test's doc comment for the full reasoning.
- [x] TASK-022 RED+GREEN: runtime shutdown while `Recovering` with a command already enqueued — assert the caller observes a terminal outcome, not a hang. Implemented as `runtime_shutdown_while_recovering_answers_enqueued_caller` using a dedicated, disposable Tokio runtime dropped before the actor's spawned task is ever polled (deterministic, not a scheduler race), matching Tokio's documented `Runtime::drop` contract.
- [x] TASK-023 RED+GREEN: 20-caller probe (`multi_thread`) under a recovery-time panic — assert all 20 `rx.await` resolve and exactly 1 actor was spawned (spawn-counter instrumentation, NFR-002). Implemented as `twenty_caller_probe_under_recovery_panic_resolves_all_and_activates_once`.
- [x] TASK-024 RED+GREEN: force an in-section construction panic and a `tokio::spawn`-outside-runtime panic for one triple — assert other triples' `entity_ref()`/`active_count()` are unaffected (poison + Round-3 deadlock safety). Implemented as two focused tests: `downcast_mismatch_never_blocks_or_disturbs_other_triples` (production-path angle on top of `registry.rs`'s existing `live_entry_is_unaffected_by_a_mismatched_lookup`, TASK-006) and `spawn_outside_runtime_panic_never_blocks_other_triples` (Round-3 self-deadlock regression, documented as a best-effort concurrent-progress proof rather than a nanosecond-exact lock-window proof, since the map lock itself is `pub(crate)` and not instrumentable from an integration test).
