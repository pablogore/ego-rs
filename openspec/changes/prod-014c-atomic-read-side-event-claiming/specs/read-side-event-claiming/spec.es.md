# Delta para read-side-event-claiming

> Documento acompañante para revisión. La fuente de verdad canónica es
> `spec.md` (identificadores 1:1). Esta es una capacidad completamente
> nueva — no existe un `openspec/specs/read-side-event-claiming/spec.md`
> previo — pero sigue la convención de este proyecto (el delta
> `production-composition-hardening` de PROD-014A) de expresar incluso una
> capacidad de primera vez como un delta de `Requisitos AGREGADOS`, para
> que `sdd-archive` la fusione de forma idéntica a cualquier otra.

Alcance: PROD-014C. Define únicamente el contrato observable de exclusión:
identidad del claim, rechazo de adquisición bajo un claim vigente, renovación
de lease, takeover basado en expiración, rechazo de dueño obsoleto mediante
fencing, liberación inmediata, preservación del orden, y la puerta de fallo
cerrado de Production. La forma exacta del puerto
(`try_claim`/`renew`/`complete`/`release` u otra), el mecanismo de
almacenamiento (tabla de claim vs. lock de advisory) y el número de
migración son decisiones de `design.md` (ver No-Objetivos).
...
- Prescribir el conjunto exacto de métodos del puerto, el mecanismo de
  ...
  de migración — decisiones de `design.md`.
