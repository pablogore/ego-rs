# API Key Authentication Specification (CORE-021)

## Purpose

Defines what the `security-apikey` crate MUST do — types, validation flow,
security invariants, and resolver contract — for opaque API key authentication
in the ego-rs security stack. All requirements describe observable behavior, not
implementation.
...
- Key generation utilities, entropy management, or prefix enforcement (`ego_sk_...`)
- Grace-period or dual-key rotation logic inside the crate (caller-owned)
- Storage adapters (Postgres, Redis, Vault)
- Authorization or scope enforcement
- Changes to `ego-domain::Credential` enum
- Changes to `security-sdk` public API
- Wire format mandates (the default parser documents a recommendation; callers may substitute)
...
| # | Question | Status |
|---|----------|--------|
| OQ-1 | Maximum byte length for `ApiKeyId`? | **Closed** — AD-6 fixes 128 chars |
| OQ-2 | Should `ApiKeyRecord` derive `Clone`? | **Closed** — Yes. `InMemoryApiKeyResolver` stores records in a `HashMap` and returns owned values; `Clone` is required. Decided in tasks phase. |
| OQ-3 | `ApiKeyResolverError` variants: `NotFound` vs `BackendError`? | **Closed** — `lookup` returns `Option`; `None` covers not-found; `ApiKeyResolverError::Backend(String)` covers backend failures. No `NotFound` variant needed. |
