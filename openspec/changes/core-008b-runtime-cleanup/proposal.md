...
1. **15 deprecated `ServiceContext::tenant_id()`/`has_tenant()` call sites** in 4 service-sdk integration test files, plus 2 call sites inside `context/mod.rs`'s own test module that exist solely to prove the deprecated accessors alias the hint correctly. Decided during proposal review: the migration window closes now — `tenant_id()`/`has_tenant()` are deleted in this change, not just silenced. Worst offender: `context_explicit_propagation.rs` — the showcase for the NEW explicit-propagation model — has 7 call sites.
