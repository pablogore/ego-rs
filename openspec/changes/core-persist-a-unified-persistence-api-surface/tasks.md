...
- [ ] 10.1 Extend `reexport_identity.rs` with witnesses for `PersistenceError`, `EventStore`, `EventStoreUnitOfWork`, `Repository`, `Snapshot`, `StoredEvent`, `resolve_tenant`, `DomainEvent` — the file now covers all 35 items (design EC-4). Fails to compile until Phase 11/12 land.

## Phase 11: GREEN — Relocate `persistence/` Files & `event.rs` — PR 3

- [ ] 11.1 Move `crates/domain/src/persistence/error.rs` verbatim to `crates/persistence-api/src/persistence/error.rs`.
- [ ] 11.2 Move `persistence/event_store.rs` verbatim (async-trait shape byte-identical, OOS-4) to `crates/persistence-api/src/persistence/event_store.rs`.
- [ ] 11.3 Move `persistence/repository.rs` verbatim to `crates/persistence-api/src/persistence/repository.rs`.
- [ ] 11.4 Move `persistence/snapshot.rs` verbatim to `crates/persistence-api/src/persistence/snapshot.rs`.
- [ ] 11.5 Move `persistence/stored_event.rs` verbatim to `crates/persistence-api/src/persistence/stored_event.rs`.
- [ ] 11.6 Move `persistence/tenant.rs` verbatim (the `resolve_tenant` three-way rule unchanged, OOS-5) to `crates/persistence-api/src/persistence/tenant.rs`.
- [ ] 11.7 Move `crates/domain/src/event.rs` verbatim (62 lines, `chrono` + `serde_json` deps) to `crates/persistence-api/src/event.rs` (AD-2).

## Phase 12: GREEN — Module Re-exports, S3 — PR 3

- [ ] 12.1 Replace `crates/domain/src/persistence/{error,event_store,repository,snapshot,stored_event,tenant}.rs` and `crates/domain/src/event.rs` with module re-exports of `ego_persistence_api::{persistence::{...}, event}`, leaving existing item-level `pub use` lines byte-identical (AD-4).

## Phase 13: Verification & Whole-Change Diff Checks — PR 3

- [ ] 13.1 `cargo build -p ego-persistence-api` succeeds standalone; `cargo build --workspace` succeeds.
- [ ] 13.2 `cargo run -p xtask -- verify-layers` passes; `cargo test --workspace` zero new failures, zero changed assertion counts across every relocated `#[cfg(test)]` module (SC-3, SC-5, SC-6).
- [ ] 13.3 Diff-read over the whole three-PR change: zero SQL/migration/schema file in the diff (SC-8, OOS-3); `crates/runtime/`, `crates/effect-store/`, and every OOS-1-named implementation struct are absent from every file list (SC-9); no crate outside `ego-domain`/`ego-persistence-api` has an edited `use` or added `Cargo.toml` dependency across all three PRs combined (SC-2).
- [ ] 13.4 Confirm KD-1 (`ProjectionStateStore`), KD-2 (`PostgreSQLRepository`'s `ON CONFLICT`/tenant defect), KD-3 (`persistent-entity/src/types.rs`), KD-4 (conformance asymmetry) are unmodified — carried, not fixed or deleted (SC-11).
- [ ] 13.5 Semantic zero-diff gate: diff public signatures and `ego_domain`'s re-export surface for every S3 item + `event.rs`, old path vs. new path — identical apart from module path. Any non-move/import/re-export change halts the PR — this is the final gate before the whole three-PR change is apply-complete (change owner gate, 2026-09-02).
