...
- [x] B5.8 Restore A3.4/A3.5's schema-index assertion and extend it to the `operation_receipts` pair.
      ...
      **Mutation evidence.** Each mutation was applied to `011_create_operation_receipts.sql`, with its anchor text proven to match exactly once before being applied, and the file restored afterwards and confirmed byte-identical by SHA-256 and `diff -q`. Exit codes are the runner's, captured rather than inferred from screen output.
      ...
      | a key column made nullable | **0 → 101** | 1 | enforcement: a nullable key exempts every NULL row, invisible to every shape check |
      | an **unregistered** table with a `LIKE`-admitted but unclassifiable predicate | **0 → 101** | 1 | discovery: nothing the sweep admits may fall between the two classifications |
      | a lopsided pair on a table in **another schema** on the search path | **0 → 101** | 1 | discovery: the sweep derives its namespaces from `current_schemas`, not a hardcoded `public` |
      | the **same table name** in two active schemas, one half in each | **0 → 101** | 1 | discovery: keyed by `(schema, table)`, so homonyms no longer merge into one apparent pair |
      | a declared nullable exception **retired** by making the column NOT NULL | **0 → 101** | 1 | enforcement: the registry is compared to the schema in both directions, so a stale exception fails |

      Every row marked `0 → 101` was **measured green** against the then-current merged code before its fix and 101 after it, with the migration restored byte-identically between runs. A claim that a mutation "would have passed" is worth nothing unless it was run; these were.

      **Round four closed the three advisory findings about the assertions' reach.** Catalog inspection is not enforcement, so `every_half_refuses_a_real_duplicate` files each probe identity twice in both partitions of all three tables and requires SQLSTATE `23505` — a refusal for any other reason fails, because "the insert failed" is not the claim. It also measures key-column nullability: a NULL never collides, so a nullable key silently exempts every row that leaves it NULL while the column list, order, uniqueness and predicate all stay identical. `events.aggregate_type` is declared as a known exception, because migration 007 makes it nullable deliberately until the operator backfill runs — recording the limit rather than implying a protection the schema does not yet give. The tenancy discriminator is excluded from that check on purpose: its nullness is what the predicates partition on, and the tenant half's own `tenant_id IS NOT NULL` already guarantees no row inside it is NULL.

      **Round five closed two defects the multi-schema sweep itself introduced.** Widening the sweep past `public` without carrying `nspname` through meant discovery keyed its map by bare `relname`, so two different tables sharing a name merged: one carrying only a tenant half and the other only a systemwide half became a single entry with both halves present, and two incomplete tables read as one complete pair. The same loss weakened the non-vacuity anchor, which a same-named table in any other namespace could satisfy on a registered table's behalf. The namespace is now part of the key, every diagnostic is schema-qualified, and the anchor resolves each registered table's effective namespace through `regclass` — following the same search-path rules the migrations followed — instead of matching on name alone. The earlier schema mutation proved *visibility* outside `public`; it never proved *isolation* between homonyms, and a new mutation does.

      The nullable-key registry is also compared in **both** directions now. Detecting an undeclared nullable column was only half the contract: a migration that makes a declared column NOT NULL retires the exception, and a one-directional check stayed green while `nullable_keys` went on advertising a hole the schema had closed. Since this registry is presented as an exact description of the guarantee's reach, a stale entry is worse than none — it misinforms a reader and would silently absorb the column becoming nullable again. A mutation that sets `events.aggregate_type` NOT NULL now fails until the declaration is removed.

      The sweep's `LIKE` and the classifier's exact equality no longer disagree: any predicate admitted by the first that matches neither exact partition predicate now fails explicitly instead of being dropped, because an index constraining part of a partition is something this file cannot reason about and must not ignore. And the namespace is measured rather than assumed — `current_schema()` and `current_schemas(false)` are read, asserted consistent, and used as the sweep's scope, so a lopsided pair on a table in any other schema on the search path is no longer invisible.
