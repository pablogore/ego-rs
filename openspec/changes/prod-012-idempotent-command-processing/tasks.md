...
- [ ] B7.11 GREEN: instrument the three spans (`idempotency.reserve`, `idempotency.takeover`, `idempotency.purge_batch`) and the counters/histogram/gauge on the existing CORE-012A/PROD-003 OTLP surface.
      ...
      **Sliced into three PRs against this tracker**, because the whole thing exceeds the 400-line review budget and the first slice is independently valuable:
      ...
      2. **The spans.** Originally scoped as "the three spans", and that turned out to be wrong on two counts once measured, so it is now three PRs covering **one** span between them.
         ...
         - **2c — `idempotency.purge_batch`**, a root span per tick (a background tick has no request boundary to descend from). Implemented and green, then held back so 2a stayed inside the review budget.
         - **`idempotency.takeover` is deliberately not emitted as a span at all.** Not deferred — decided. `Tracer::start_span` fixes both the name and the start instant at the call, and the takeover is only known *after* `reserve` returns, so the only ways to honour the table were a zero-duration marker span (misrepresenting exactly what a span exists to represent) or a span with fabricated timestamps. From the runtime's vantage point there is one timed operation: the takeover happens inside the store, which returns a single outcome, so AD-10's two-span design presumes visibility the runtime does not have. Its observability comes from `idempotency.reservation.outcome` with `outcome=taken_over`, which the same spec already requires — no information is lost, and the deviation is in the span table's shape, not its content.
