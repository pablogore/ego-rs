...
- [ ] B7.11 GREEN: instrument the three spans (`idempotency.reserve`, `idempotency.takeover`, `idempotency.purge_batch`) and the counters/histogram/gauge on the existing CORE-012A/PROD-003 OTLP surface.
      ...
      **Measured before implementing, and AD-10 assumes a richer surface than exists.** Recorded here because it changes what these two boxes can honestly claim.
      - `Observability::metric(&self, name: &str, value: f64)` has **no attribute parameter**, so none of AD-10's metric attributes (`reason`, `carrier`, `outcome`, `event`, `operation`, `aggregate_type`) is expressible as written, and there is no separate histogram or gauge kind. Every attribute in that table is a bounded enum, so they are emitted folded into the metric name instead — a deliberate, documented deviation from the table's shape, not from its content.
      - `metric` has **zero production call sites** in the workspace; its only implementations are `NoopObservability` (empty body) and test spies. There is no OTLP *metrics* exporter — the spans side has one (`crates/infrastructure/src/tracing_otlp.rs`), the metrics side does not. So "verify the telemetry actually emitted" can only mean against a recording double for the counters, and that is stated rather than implied.
      - `SpanAttributes` was a closed two-field allow-list whose own docs call it "the ONLY way to attach attributes to a span", with redaction "enforced structurally at this type rather than by a runtime filter" (ADR-6). `idempotency.operation_key_hash` was therefore **not expressible**, and admitting it is a deliberate widening of that list — decided explicitly rather than assumed, and kept to one bounded curated concept, the way `tenant_present` already is.
      - Two of the three properties asked for hold **by construction, not by test**: the raw key is not expressible as a span attribute, and nothing can be a metric attribute at all. There is no runtime filter to mutate. What *is* mutable, and is guarded, is the hash computation and the adapter's mapping.
      **Sliced into three PRs against this tracker**, because the whole thing exceeds the 400-line review budget and the first slice is independently valuable:
      1. **The redaction primitive** (368 changed lines). `OperationKeyHash` in `crates/domain/src/operation/key.rs` — sole constructor hashes an `OperationKey`; `SpanAttributes::with_operation_key_hash` takes that type, never a `String`; the OTLP adapter maps it to `idempotency.operation_key_hash`. Ordered first on purpose: the redaction type lands before any instrumentation exists that could emit a key, so there is never a window in between. Six mutations measured — truncating to 8 hex, uppercase hex, emitting the raw key, hashing a constant, dropping the attribute before export, and renaming the attribute — each kills at least one test. `sha2` added to `ego-domain`: the rule is a domain rule (CORE-019 §12), and a hash computed in an adapter could be bypassed by a caller that already has a string.
      ...
      2. **The three spans.** Blocked on plumbing, which is why it is its own slice: **`RuntimeInner` holds no tracer.** The tracer is consumed at `build()` solely to construct a `TracingInterceptor` and is not retained, so `reserve_idempotent_operation` has nothing to open a span with. Retaining it means a new field through `new_with_logger`.
