...
- [x] B7.6 **(satisfied by table separation)**: no code to correct. The purge query never named `operation_receipts`, and nothing relates the two tables, so the guarantee held before this task was opened.
      **Guarded at its precondition, not its behaviour.** `migrations.rs`'s test module gains one bounded assertion — the receipts migration contains no `REFERENCES`, `FOREIGN KEY` or `CASCADE` — checked against the migration text the crate already embeds via `include_str!`, so it is a substring check rather than SQL parsing and needs no infrastructure. Measured: appending a cascading foreign key to that migration fails it.
