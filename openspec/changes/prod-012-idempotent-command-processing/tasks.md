...
- [x] B7.2 GREEN **(reframed by B3-i)**: harden the already-implemented `purge_completed_before` for production — batched execution that is observable, and safe for multiple concurrent workers. B3-i implements a correct single-worker purge satisfying the port's four guarantees (eligibility strictly before the cutoff, never an `InProgress` row, the batch limit, the returned count); what remains here is multi-worker safety and instrumentation, not the implementation.
- [x] B7.3 RED: two-concurrent-workers test — overlapping eligible rows purged exactly once, no deadlock, no double-purge.
- [x] B7.4 GREEN: implement the concurrency-safe purge query (e.g. `SELECT ... FOR UPDATE SKIP LOCKED` or equivalent row-claiming pattern).
  **B7.2/B7.3/B7.4 closing note — a progress guarantee, not concurrency safety.** The task text called this multi-worker *safety*; measuring first showed that was false. The pre-existing query was already safe: two workers cannot remove the same row twice, because the second `DELETE` blocks on the row lock, re-evaluates under READ COMMITTED, finds the row gone and removes zero. PostgreSQL provides that, not our SQL — so "no row deleted twice" and "counts sum to rows removed" hold either way and were deliberately **not** used as evidence. An assertion that cannot distinguish the two implementations is not evidence about either.
  **What was actually broken was progress.** Measured with two of four eligible rows held by another transaction and a batch of two: the statement waited on a locked tuple until its timeout while two free eligible rows sat untouched — `canceling statement due to statement timeout … while deleting tuple (0,2)`. That is head-of-line blocking. It is **not** a deadlock: no circular wait was reproduced, and "it could happen" was not treated as a requirement.
  **Landed** as `integration-tests/tests/purge_progress_postgres.rs` — one durable test, a forced window rather than a race, asserting the batch is filled from unlocked rows within a deadline, the held rows and the ineligible row survive, and a later call drains the held rows once released (skipping is deferral, not exclusion). `FOR UPDATE SKIP LOCKED` added to the selection subquery; removing it makes that test fail on its deadline.
