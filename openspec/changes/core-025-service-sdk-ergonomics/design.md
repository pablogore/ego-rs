...
- **Resolution only exists on `Runtime`, never on `RuntimeBuilder`.** There is no `RuntimeBuilder::resolve` and none is planned — `resolve::<Tag>()` requires a built `Runtime` (it reads `RuntimeInner.registry`, which only exists post-`build()`/`try_build()`). Registration (`with_service`) and resolution (`resolve`) are on two different types precisely so this can't be gotten wrong by accident: the type signatures alone rule out "resolve mid-build."
- **`resolve::<Tag>()` is not cached — each call constructs a fresh `{Trait}Ref`.** The underlying `Arc<dyn Trait>` inside the registry entry is the same instance every time (cloned, not reconstructed), but the proxy wrapper (`{Trait}Ref { inner, chain, runtime }`) is a new value on every `resolve()` call — a few cheap `Arc` clones and one `Arc::downgrade`, not a cache lookup. This matches the hand-rolled path exactly (calling `{Trait}Ref::new(...)` twice today also produces two distinct proxy values over the same inner service). Intended usage is to `resolve()` once and hold onto the returned proxy for repeated invocations, same as a caller would do with the hand-rolled constructor — calling `resolve()` again per-invocation is not incorrect, just unnecessary allocation. Put plainly: `resolve()` clones the already-registered `Arc`-backed proxy; it never reconstructs the underlying implementation.
...
**Normative — validation order:** recorded validators run in **registration order** — the exact order `with_injectable::<S>()` calls were chained on the builder — not an unordered or hash-based iteration. This is a guarantee, not an incidental property of today's `Vec<ValidatorEntry>` storage: if the storage ever changes, registration-order execution must be preserved, because callers can and do rely on "the first missing dependency in build order" being the one reported when several are absent. A future change that reorders validators (e.g. sorting by type name) would be a breaking change to error reproducibility and must not happen silently.
...
**Standing invariant (the whole reason B exists — must not erode over time):** `validate()` MUST remain a pure presence check against `DependencyTable` and nothing else — no allocation beyond what `check_dependency`'s own error path needs, no I/O, no external calls, no partial construction. The default implementation satisfies this by construction (it only calls `dependencies()` and `check_dependency`). Because `validate()` is a **defaulted, not sealed** trait method, a future hand-rolled `Injectable` impl *could* override it with something heavier — that must not happen. If a future need ever requires validation logic beyond "does this dependency exist" (e.g. "is this connection string well-formed"), that logic belongs in `build()`'s own construction path (surfaced via the existing resolution-path `DependencyNotFound`/other errors), not folded into a widened `validate()` override. Letting `validate()` creep toward doing what `build()` does is exactly the two-constructors failure mode this design change exists to avoid — the risk is not hypothetical, it is the specific thing AD-3 was revised to prevent, so it must be actively guarded against in code review for any future `Injectable` impl, not just assumed away by the default's current shape.
