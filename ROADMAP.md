...
* [ ] Improve composition error diagnostics — **small atomic cleanup** (2026-08-24 spike): `DependencyNotFound`/`ServiceNotFound` already carry actionable fix hints; the CORE-028D1 `Duplicate*` variants don't. Must preserve D1's decisions: `DuplicateAdapter` may reference `.replace_adapter()`, but `DuplicateEffectStore`/`DuplicateEffectRetentionStore` must NOT suggest a replace escape hatch (deliberately none exists) — message should read closer to "register exactly one effect store." Likely a direct PR (`Display` text + tests), not necessarily an OpenSpec.
* [ ] Improve startup diagnostics — **REAL GAP, next work (CORE-028D4)** (2026-08-24 spike): `App::start()` has zero structured logging/tracing; `start_effects()` always returns `Ok(())` so the documented `CompositionError::Startup` failure path is effectively dead. Scope: emit structured start/started/failed signals with elapsed duration and closed error classification, no lifecycle/ownership change, no health/readiness (that's PROD-005).
...
* [ ] Review projection spawning ergonomics — **REAL GAP, spike before spec** (2026-08-24 spike): ~130 lines of hand-rolled scheduler wiring in `reference-app/src/read_side/mod.rs`, no framework support. CORE-028D2 already fixed lifecycle as host-owned — do not reopen that via a `spawn_projection()` shortcut. Next step is a scoped spike ("what part of the ~130 lines is repeatable framework boilerplate vs. legitimate host config") with explicit permission to conclude DEFER/DROP, same as D3 — there is currently only one real projection consumer.
