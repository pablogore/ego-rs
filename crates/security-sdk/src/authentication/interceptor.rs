...
mod tests {
    ...
    fn ok_security_context() -> SecurityContext {
        let principal = Principal::new(PrincipalKind::User, SubjectId::new("user-1").unwrap());
    ...
    fn intercept_populates_security_context_on_valid_credential() {
        let interceptor =
            AuthenticationInterceptor::new(Arc::new(AlwaysBearerExtractor), Arc::new(OkProvider));
        let ctx = MapCtx::new(&[]);
    ...
    fn intercept_passes_through_when_no_credential() {
        let interceptor =
            AuthenticationInterceptor::new(Arc::new(NoCredentialExtractor), Arc::new(OkProvider));
        let ctx = MapCtx::new(&[]);
        let mut set_called = false;
        interceptor
            .intercept(&ctx, |_| {
        ...
        assert!(
            !set_called,
            "set_security must not be called with no credential"
        );
    }
    ...
    fn intercept_propagates_extractor_error() {
        let interceptor =
            AuthenticationInterceptor::new(Arc::new(FailingExtractor), Arc::new(OkProvider));
        let ctx = MapCtx::new(&[]);
