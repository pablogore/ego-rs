//! Persistence SPI — domain-owned contracts for event sourcing (CORE-PERSIST-A
//! S3). Relocated verbatim from `ego_domain::persistence::{error,event_store,
//! repository,snapshot,stored_event,tenant}`; `ego-domain` re-exports each
...
//! | `tenant` | `resolve_tenant` — resolution of a caller-supplied tenant scope into the value a store files under |
