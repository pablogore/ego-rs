...
/// DropOldest currently falls back to DropNewest (TODO #79).
/// Sending to a full bus must return Ok(()) — not an error.
#[test]
fn drop_oldest_accepts_new_event_when_full() {
    let config = EventBusConfig {
        capacity: 2,
    ...
    tx.try_send(env1).unwrap();
    tx.try_send(env2).unwrap();
    // Bus is full — DropOldest must not return an error.
    assert!(tx.try_send(env3).is_ok());

    // Two original events remain in the buffer (DropNewest fallback behaviour).
    let items = rx.drain_all();
    assert_eq!(items.len(), 2);
}
